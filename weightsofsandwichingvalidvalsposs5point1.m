function finalweighteddata = weightsofsandwichingvalidvalsposs5point1(inputtimes,inputdata,curdescleantime,i,j,missingdataval)
%From an index of interest in an time-denominated inputvec, looks for valid values both before and after
%   Built for 'possibility 5.1' in testvaluesatstandardtimes
%   If no valid value can be found on either of the sides, both weights are set to a missing value

%For testing:
%inputtimes=[22;1];
%inputdata=[14;8];
%descleantimes=[23]; %on the first of the 3 days spanned
%i=1 -- the index of descleantimes
%therefore curdescleantime=23;
%j=2 -- the index of inputtimes corresponding to the latter half of the pair between which curdescleantime is found
...
if j>=2
    if abs(inputdata(j-1))<missingdataval
        greenlightnumber1=1;
        distearliertime=curdescleantime-inputtimes(j-1);
...
if greenlightnumber1==1
    if j<=size(inputtimes,1)
        if abs(inputdata(j))<missingdataval
            greenlightnumber2=1;
            distlatertime=inputtimes(j)+24-curdescleantime;
...
if greenlightnumber1==1 && greenlightnumber2==1
    %Adjust distances for the circumstances of this case
    distlatertime=distlatertime+24;
