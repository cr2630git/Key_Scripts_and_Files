function[fillhandle,msg]=shadebetweencurves(xpoints,upper,lower,color,edge,add,transparency)
%USAGE: [fillhandle,msg]=shadebetweencurves(xpoints,upper,lower,color,edge,add,transparency)
...
%     [ph,msg]=shadebetweencurves(x,a,b,rand(1,3),rand(1,3),0,rand(1,1))
%     grid on
%     legend('Datr')
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    msg='Error: Must use the same number of points in each vector';disp(msg);
end
