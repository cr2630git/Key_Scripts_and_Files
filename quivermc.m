function [hvectors,cb,normrefveclength] = quivermc(lat,lon,u,v,whethertoaddtext,varargin)
% QUIVERMC is an adapted version of Andrew Roberts' ncquiverref. 
% Both functions fix a couple of problems with Matlab's quiverm function.
...
%%%% RUNTIME OPTIONS %%%%
troubleshoot=0; %whether to display various troubleshooting messages
%%%%                 %%%%
...
%whethertoaddtext controls whether text and the reference vector will be added on to the plot
%using the text and annotation functions (fine for screenshots but not great for presentation-quality figures)
%anything other than 'addtext' doesn't add them and lets another later script such as highqualityfiguresetup do the work
%
%
%% Examples: 
% 
...
reftype = 'max'; 
tmp = strncmpi(varargin,'refer',5) | strncmpi(varargin,'scale',5); 
if any(tmp) 
...
map_area=varargin{2};
%disp('line 241');disp(map_area);disp(skipstep);
...
extraskipstepfactor=varargin{6};%disp('line 251');disp(extraskipstepfactor);disp(skipstep);
lat = lat(1:extraskipstepfactor:end,1:extraskipstepfactor:end); 
lon = lon(1:extraskipstepfactor:end,1:extraskipstepfactor:end);
u = u(1:extraskipstepfactor:end,1:extraskipstepfactor:end); 
v = v(1:extraskipstepfactor:end,1:extraskipstepfactor:end); 

refvectorshrinkfactor=varargin{8};%disp('line 256');disp(refvectorshrinkfactor);
...
%Vectors can use the same reference vector
%Don't project the length of the vector to be different in x and y directions
[th,z] = cart2pol(u,v);
...
%Scale the vectors according to the mean distance between gridpoints
%This is a good measure, as it remains 
...
distbwgridpts=min(mean(z1(~isnan(z1))),mean(z2(~isnan(z2))));
if troubleshoot==1;fprintf('Mean distance between gridpts is %d\n',distbwgridpts);end
...
%Remove NaN values that will not be plotted and turn points into a row of coordinates
u=u(~isnan(x))';v=v(~isnan(x))';
y=y(~isnan(x))';x=x(~isnan(x))';

%Set arrow size (3=full length of vector)
arrow=0.35;

%Set scale value based on refval, scale length, and map area
%larger scale --> bigger arrows representing the same value, all else being equal
...
scaleval=(distbwgridpts/refval)/24*sq_map_area*(skipstep/extraskipstepfactor);
if troubleshoot==1
    fprintf('Refval is %d\n',refval);fprintf('Sq_map_area is %d\n',sq_map_area);
    fprintf('Skipstep is %d\n',skipstep);fprintf('Extraskipstepfactor is %d\n',extraskipstepfactor);
    fprintf('Scaleval is %d\n',scaleval);
end


%Adjust whether arrows are centered over grid points or whether they converge/diverge
switch arrowStyle
    case 'centered' %default
        xstart=x-0.5*scaleval*u;
        xend=x+0.5*scaleval*u;
        ystart=y-0.5*scaleval*v;
        yend=y+0.5*scaleval*v;
    case 'divergent'
        xstart=x;
        xend=x+scaleval*u;
        ystart=y;
        yend=y+scaleval*v;
    case 'convergent' %i.e. arrows converge upon a grid point
        xstart=x-scaleval*u;
        xend=x;
        ystart=y-scaleval*v;
        yend=y;
...
%Get x coordinates of all the different parts of each vector plotted
lx = [xstart;x;...
    xstart+(1-arrow/3)*(xend-xstart);xend-arrow*(scaleval*u+arrow*(scaleval*v)); ...
  xend;xend-arrow*(scaleval*u-arrow*(scaleval*v)); ...
  xstart+(1-arrow/3)*(xend-xstart);NaN(size(x))];

%Get y coordinates of all the different parts of each vector plotted
ly = [ystart;y;...
    ystart+(1-arrow/3)*(yend-ystart);yend-arrow*(scaleval*v-arrow*(scaleval*u)); ...
  yend;yend-arrow*(scaleval*v+arrow*(scaleval*u)); ...
  ystart+(1-arrow/3)*(yend-ystart);NaN(size(y))];
...
if exist('cmap','var') 
    ...
    for k=1:length(magnitude(:));set(hvectors(k),'Color',cmap(magind(k),:));end
else
    cb=0;
end
 ...
 xdist=refval*(scaleval/refvectorshrinkfactor);
 if troubleshoot==1;fprintf('Refvectorshrinkfactor is %d; final ref-vector length is %d\n',refvectorshrinkfactor,xdist);end

 %Need to normalize ref vector by size of plot relative to size of figure, otherwise
 %the other vectors will be shrunken (elongated) but the reference vector won't, making it comparatively too large (small)
 %The factor of 2.6 below is obtained by the following method:
 %1. the observation that for maps the x-limits are approximately [0 1.6] while for the entire figure they are [0 1] (in normalized units)
 %2. noting that for a figure opened to full size on the screen (with about 1" margins on the left & right sides),
    %the figure measures 26 cm while the map measures only about 16 cm
 %3. consequently, for the plot 1 cm=0.1 units while outside the plot, in the broader figure, 2.6 cm=0.1 units
 %4. this means that to make something appear the same size in both, 
    %its size in numbers must be x/2.6 outside the plot relative to x within it
 %5. visually check/measure to confirm this is the case if any doubts surface
 normrefveclength=xdist/2.6;
 if troubleshoot==1;fprintf('Original xdist is %d; normalized ref vec length is %d\n',xdist,normrefveclength);end
 
 map_region=varargin{4};%disp('line 482');disp(map_region);
 if strcmp(map_region,'north-america')
     disthere=0.92;
 else
     %disthere=0.84;
     disthere=0.92;
 end
 if strcmp(whethertoaddtext,'addtext')
     x=[disthere-normrefveclength disthere]; %start and end of arrow respectively
     y=[0.43 0.43];
     ...
     B=uicontrol('Style','text','String',phr,'Units','normalized',...
         'Position',[disthere-xdist+0.01 y(1)-0.05 0.11 0.03],'FontName','Arial','FontSize',14);
...
if changelinewidth
    ...
    if refvec;set(hrefvec,'linewidth',lineWidth);end
end
    

%% Clean up: 

if nargout==0;clear hvectors;end
end

%%%  End of quivermc function  %%%
